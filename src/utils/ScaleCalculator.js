...
export class ScaleCalculator {
  ...
  static formatScale(meters) {
    ...
    return `${sign}${mantissa.toFixed(2)} × 10${ScaleCalculator.toSuperscript(exponent)} m`;
  }

  /**
   * Convert a number (including negative) to Unicode superscript characters
   *
   * @param {number|string} n - The number to convert
   * @returns {string} Unicode superscript representation
   */
  static toSuperscript(n) {
    ...
    return String(n).split('').map(ch => superscriptMap[ch] || ch).join('');
  }
